...
async def sendConfigAndInstructions(chatId, device='iPhone', type='xui'):
    ...
    if type == 'xui':
        ...
        if connectionLinks['success']:
            ...
            additionalText = ("\r\n\r\nЕсли вы подключаетесь <b>в первый раз</b> или <b>после активации уже неактивной подписки</b>, конфигурация вашего ключа может дойти на все серверы не сразу, а с задержкой до 10 минут." \
                              "Поэтому, если не работает впн при подключении в первый раз или после активации уже неактивной подписки, попробуйте подождать 10 минут и переподключиться (ключ добавлять по-новой не нужно, достаточно просто переподключить приложение)")
