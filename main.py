...
async def Work_with_Message(m: types.Message):
    ...
    if m.from_user.id in CONFIG["admin_tg_id"]:
        ...
        if e.demojize(m.text) == "Отправить всем сообщение о подарках :pencil:":
            ...
            dbCur = conn.cursor(pymysql.cursors.DictCursor)
            dbCur.execute(
                f"SELECT * from userss where UNIX_TIMESTAMP(date_create) > 1734248409 ORDER BY id ASC;")
            log = dbCur.fetchall()
            dbCur.close()
            conn.close()
            ...
            for i in log:
                try:
                    ...
                    await bot.send_message(i['tgid'], emoji.emojize(f"<b>Что дарить на Новый Год?</b>🎅\r\n\r\n" \
                                                                    f"Друзья! До нового года осталось меньше 2х недель, а это значит что пора позаботиться о подарках коллегам, друзьям и родным ♥️\r\n\r\n" \
                                                                    f"Мы запустили в продажу подарочные сертификаты на наш впн. Период подписки вы можете выбрать любой 🙌🏻\r\n\r\n" \
                                                                    f"Купить подарочную подписку вы можете уже сейчас, а подарить хоть через год, так как подписка активируется в момент, когда вы отправляете персональную ссылку получателю.\r\n\r\n" \
                                                                    f"Для того чтобы подарить подписку и узнать подробности, нажмите на кнопку <b>Подарить подписку на НГ</b> 🎅\r\n\r\n" \
                                                                    f"Небанальный и актуальный подарок для коллег и близких🎄"),
                                           parse_mode="HTML",
    ...
    if "Подарить подписку" in m.text:
        await bot.send_message(chat_id=user_dat.tgid,
                               text=e.emojize(f"<b>Подарите подписку друзьям, коллегам и родным</b> 🎁" \
                                              ...
                                              f"\n\r\n\r2. После ответным сообщением мы отправим вам сертификат с новогодним дизайном, ссылку и секретный код, которые вы отправите получателю подарка (ссылка и секретный код действуют неограниченное количество времени)." \
                                              f"\n\r\n\r3. Когда получатель подарка перейдет по ссылке и введет секретный код, у него активируется подарочная подписка."
                                              ),
                               parse_mode="HTML")
    ...
    if e.demojize(m.text) == "Помощь :heart_hands:":
        ...
        helpButtons.add(
            ...
            types.InlineKeyboardButton(e.emojize("Подарить подписку на НГ 🎅"), callback_data="Help:GIFT"),
        )
...
def checkTime():
    ...
    while True:
        try:
            ...
            # dbCur = conn.cursor(pymysql.cursors.DictCursor)
            # dbCur.execute(
            #     f"SELECT * FROM userss WHERE TIME(date_create) > TIME(CURRENT_TIME() - INTERVAL 5 MINUTE) ORDER BY id DESC LIMIT 10;")
            # log = dbCur.fetchall()
            # dbCur.close()
            # conn.close()
            #
            ...
            # dbCur = conn.cursor(pymysql.cursors.DictCursor)
            # dbCur.execute(
            #     f"SELECT * FROM payments WHERE TIME(time) > TIME(CURRENT_TIME() - INTERVAL 5 MINUTE) ORDER BY id DESC LIMIT 10")
            # log = dbCur.fetchall()
            # dbCur.close()
            # conn.close()
            #
