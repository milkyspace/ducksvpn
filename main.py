    ...
    if "Подарить подписку" in m.text:
        await bot.send_message(chat_id=user_dat.tgid,
                               text=e.emojize(f"<b>Подарите подписку друзьям, коллегам и родным</b> 🎁" \
                                              ...
                                              f"\n\r\n\r2. После этого ответным сообщением мы отправим вам сертификат с новогодним дизайном, ссылку и секретный код, которые вы отправите получателю подарка (ссылка и секретный код действуют неограниченное количество времени)." \
                                              f"\n\r\n\r3. Когда получатель подарка перейдет по ссылке и введет секретный код, у него активируется ваша подаренная подписка."
                                              ),
                               parse_mode="HTML")
